...
public class Main extends Application {
	...
	private Scene startScene() {
		...
		homePage.setText("Home Page" + "\nNumber of question in database: " + Integer.toString(Quiz.numberOfQuestion));
	...
	private Scene quizSelectionScene() {
		...
		ObservableList<String> topicOptions = FXCollections.observableArrayList("topic1", "topic2", "topic3", "topic4");
	...
	private Scene quizAnsweringScene() {
		...
		addMoreQuestion.setOnAction(e -> windows.setScene(quizAnsweringScene()));
		...
		Label quesAll = new Label("Total Question: " + Quiz.numberOfQuestion);
	...
	private Scene saveQuizResult() {
		...
		exit.setOnAction(
				e -> {
				    ...
				    alert.getDialogPane().lookupButton(confirmExit).addEventFilter(ActionEvent.ACTION, event -> 
				        windows.setScene(goodByePage()));
				    alert.getDialogPane().lookupButton(cancel).addEventFilter(ActionEvent.ACTION, event -> 
